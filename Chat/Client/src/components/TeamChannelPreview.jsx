...
const TeamChannelPreview = ( setActiveChannel ,setIsCreating, setIsEditing ,setToggleContainer ,{channel, type}) => {
    ...
    const ChannelPreview = () => ( // Get the name or the ID from the channel
        <p className="channel-preview__item">
            # {channel?.data?.name || channel?.data?.id}
        </p>
    );

    const DirectMessages = () => { // Get the messages from the current members in the chat
    ...
    return (
        <div className={channel?.id === activeChannel?.id ? "channel-preview__wrapper__selected" : "channel-preview__wrapper"} onClick={() => {
            ...
            {type === "team" ? <ChannelPreview /> : <DirectMessages />}
        </div>
