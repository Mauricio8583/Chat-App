...
const channelByUser = async ({ client, setActiveChannel, channel, setChannel }) => { // Return the channels that the user is active and add new ones
  const filters = {
    type: 'messaging',
    member_count: 2,
...
const SearchResult = ({ channel, focusedId, type, setChannel, setToggleContainer }) => {
  ...
  if (type === 'channel') {
    return (
      ...
      </div> // Show the result according to the channel
    );
  }
